...
const SVGpart01 = `<svg className="rounded-full shadow-xl w-96" xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' viewBox='0 0 2500 2500'><defs>
...
export default function Home() {
  ...
  const [colorTrait, setColorTrait] = useState(colorTraits[1]);
  const [headTrait, setHeadTrait] = useState(0);
  ...
  function App() {
    const T = parse(html);
    return <div className="flex justify-center border mx-auto">{T}</div>;
  }
  ...
  function upArrow2() {
    ...
    setColorTrait(colorTraits[colorCounter]);
    setColorName(colorNames[colorCounter]);
    console.log("After");
    console.log(colorCounter);
  }
  ...
  return (
    <div className="border w-screen flex mx-auto flex-col justify-center">
      <svg
        className="w-1/2 border flex mx-auto"
        xmlns="http://www.w3.org/2000/svg"
      ...
      >
        <defs>
          ...
          >
            ...
            <stop offset={1} stopColor="#f9d559" />
          </linearGradient>
          <style>
            {
              ".bg,.body,.underbody,.nostrils,.eyes,.pupils,.mouth,.teeth,.fins,.gills{stroke:#606060;}.body,.underbody,.nostrils,.eyes,.pupils,.mouth,.teeth,.fins,.gills{stroke-width:5px;}.bg,.nostrils,.eyes,.pupils,.mouth,.teeth,.fins,.body,.underbody,.gills{stroke-linecap:round;stroke-linejoin:round;}.bg{stroke-width:5px;fill:url(#linear-gradient);}.bg,.body,.underbody,.nostrils,.eyes,.pupils,.mouth,.teeth,.fins,.gills,.shadow{fill-rule:evenodd}"
            }
            {colorTrait}
          </style>
        </defs>
      ...
      <div className="flex mx-auto text-2xl">
        <button onClick={upArrow1}>⬅</button>
        {colorName}
        <button onClick={downArrow1}>➡</button>
        {count}
        {(function () {
          if (colorCounter === 0) {
            return <button>Logout1</button>;
          } else if (colorCounter === 2) {
            return <button>Logout2</button>;
          } else if (colorCounter === 3) {
            return <button>Logout3</button>;
          } else if (colorCounter === 4) {
            return <button>Logout4</button>;
          } else if (colorCounter === 5) {
            return <button>Logout5</button>;
          }
        })()}
        TEST
      </div>
